...
import com.googlesource.gerrit.plugins.replication.SecureCredentialsProvider;
...
public class AutoReloadSecureCredentialsFactoryDecorator implements CredentialsFactory {
  ...
  public AutoReloadSecureCredentialsFactoryDecorator(
      SitePaths site, ReplicationFileBasedConfig config, SecureCredentialsFactory secureCredentialsFactory) {
    this.site = site;
    this.config = config;
    this.secureCredentialsFactory = new AtomicReference<>(secureCredentialsFactory);
    ...
    try {
      if (needsReload()) {
        secureCredentialsFactory.compareAndSet(
            secureCredentialsFactory.get(), secureCredentialsFactory.get());
